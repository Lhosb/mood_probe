RSpec.describe MoodProbe::Extractor do
  ...
  it "returns positionally aligned results for mixed good and bad input" do
    ...
    results = extractor.analyze_all(paths, descriptors: [:mood_happy])
  ...
  it "raises configuration errors before any file is processed" do
    ...
    expect { extractor.analyze_all(%w[one.wav two.wav], descriptors: [:mood_happy]) }
  ...
  it "still performs configuration preflight for an empty input list" do
    ...
    expect { extractor.analyze_all([], descriptors: [:bpm]) }
  ...
  it "retries descriptor verification after a failed plan preflight" do
    ...
    expect { extractor.verify!(descriptors: [:mood_happy]) }
      .to raise_error(MoodProbe::BackendError, /preflight failed/)

    allow(backend).to receive(:preflight_plan!).and_return(true)
    expect(extractor.verify!(descriptors: [:mood_happy])).to be(true)
  ...
  it "propagates fatal BackendError and stops processing later paths" do
    ...
    expect do
      extractor.analyze_all(%w[one.wav two.wav three.wav], descriptors: [:mood_happy])
  ...
  it "keeps positional results when one file returns a malformed scalar" do
    ...
    expect(results.values_at(0, 2).map { |result| result.analysis[:mood_happy].value })
      .to eq([0.1, 0.4])
  end
end
